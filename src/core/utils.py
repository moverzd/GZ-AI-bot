...
def strip_markup(text: str) -> str:
	"""
	Удаляет HTML и Markdown-разметку, сохраняя только текст.
	- Сохраняет списки: нормализует маркеры в начале строки к "- "
	- Убирает HTML-теги
	- Убирает Markdown-маркеры (** __ * _ ` ``` #)
	- Сохраняет содержимое внутри маркеров
	"""
	if not text:
		return ""
	s = text
	# Удаляем HTML-теги
	s = re.sub(r'</?[^>]+>', '', s)
	# Нормализуем маркеры списков в начале строк: *, • -> -
	s = re.sub(r'(?m)^[\t ]*[\*•][\t ]+', '- ', s)
	# Сохраняем нумерованные списки как есть
	# Убираем кодовые ограждения, сохраняя содержимое
	s = s.replace('```', '')
	s = s.replace('`', '')
	# Удаляем маркеры жирного/курсива, сохраняя содержимое
	s = re.sub(r'\*\*(.*?)\*\*', r'\1', s)
	...
	# Убираем оставшиеся одиночные маркеры, но не трогаем дефисы у списков
	# Заменяем одиночные символы подчёркивания
	s = s.replace('_', '')
