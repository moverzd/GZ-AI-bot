...
3. Простое текстовое форматирование: Используй только простое текстовое форматирование, подходящее для Telegram.
   - Списки оформляй через дефис "-" или нумерацию "1.".
   - Активно используй переносы строк и отступы для структурирования.
   - ЗАПРЕЩЕНО использовать любую Markdown-разметку: не используй **жирный шрифт**, *курсив*, ## заголовки и т.д.
...
показатели, Требования к сырью и материалам, Маркировка, Упаковка, Требования
...
USER_PROMPT = """
Ответь на вопрос, используя ТОЛЬКО предоставленные документы. Соблюдай все системные инструкции. Особенно:
- Начинай ответ с главного, без лишних вступлений.
- Указывай источник для ключевых данных (СТО, Паспорт качества) и метод испытания (ГОСТ) для каждого показателя.
- Форматируй ответ как простой текст без Markdown, строго следуя шаблону из системного промта.
- Объясняй технические термины при первом упоминании.
- Если информации нет - сразу сообщи.
ДОКУМЕНТЫ:{context}

ВОПРОС: {query}

ОТВЕТ (только на русском, только по документам, строгое форматирование из системного промта):
"""
    ...
    async def generate_response(self, query: str, search_results: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                temperature=0.2,  # Более консервативная температура для точности
                max_tokens=1200,  # Увеличенный лимит токенов
