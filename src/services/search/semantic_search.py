...
class SemanticSearchService(BaseSearchService):
    ...
    async def find_products_by_query(
        ...
        Args:
            ...
            user_id: ID пользователя (не используется в семантическом поиске)
            limit: Максимальное количество результатов
            
        Returns:
            Список найденных продуктов, отсортированных по релевантности
        """
        try:
    ...
    async def _fetch_products_by_similarity_results(
        ...
        Args:
            similar_products: Результаты семантического поиска
            category_id: ID категории для дополнительной фильтрации
    ...
    def _sort_products_by_relevance(
        ...
        Args:
            ...
            similar_products: Результаты семантического поиска с оценками
